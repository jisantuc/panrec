module TypescriptInterfaceSpec
      ...
      TS.parseInterface exampleInterface `shouldBe`
        (Right $
         TS.Interface
           ...
           , ( "enemies"
             , VendorHK "ReadonlyArray" [Vendor "relationships.Enemy"])
           ]
           "Character")
    it "should parse and encode and parse an interface correctly" $
      do do parsed <-
              parseRecords (emptyR :: TS.Interface) exampleInterfaceWithKW
            let encoded = constructor <$> parsed
            let concatted = (++ "\n") $ mconcat $ intersperse "\n" encoded
            parseRecords (emptyR :: TS.Interface) (pack concatted)
         `shouldBe` (Right $
                     [ TS.Interface
                         [ ("id", String')
                         , ("name", String')
                         , ( "friends"
                           , VendorHK "ReadonlyArray" [Vendor "Friend"])
                         , ( "enemies"
                           , VendorHK
                               "ReadonlyArray"
                               [Vendor "relationships.Enemy"])
                         ]
                         "Character"
                     ])
